...
import de.nx42.maps4cim.gui.util.ParseAllFormat;

/**
 * Extension of {@code JFormattedTextField} which solves some of the usability
 * issues
 */
public class JFormattedTextFieldColored extends JFormattedTextField {
    
    private static final long serialVersionUID = -7123007203807432427L;
    
    protected static final Color ERROR_BACKGROUND_COLOR = new Color(255, 215, 215);
    protected static final Color ERROR_FOREGROUND_COLOR = null;
    
    protected Color fBackground, fForeground;
    
    /**
     * Create a new {@code ImprovedFormattedTextField} instance which will use
     * {@code aFormat} for the
     * validation of the user input.
     * 
     * @param aFormat The format. May not be {@code null}
     */
    public JFormattedTextFieldColored(Format aFormat) {
     ...
     * Create a new {@code ImprovedFormattedTextField} instance which will use
     * {@code aFormat} for the
     * validation of the user input. The field will be initialized with
     * {@code aValue}.
     * 
     * @param aFormat  The format. May not be {@code null}
     * @param aValue The initial value
     */
    public JFormattedTextFieldColored(Format aFormat, Object aValue) {
        this(aFormat);
        setValue(aValue);
    }
    
    public JFormattedTextFieldColored(AbstractFormatter formatter) {
        super(formatter);
        init();
    }
    ...
    protected boolean processKeyBinding(KeyStroke ks, KeyEvent e, int condition, boolean pressed) {
        ...
        // to trigger an OK button by pressing enter from within the formatted
        // text field
        if (validContent()) {
    ...
    protected void processFocusEvent(FocusEvent e) {
        ...
        // fires an empty keyreleased-event, when focus is lost
        // useful to notify observers which are in illegal state AFTER problems are healed
        for (KeyListener listener : getKeyListeners()) {
            listener.keyReleased(null);
        }
    }

    protected static class MousePositionCorrectorListener extends FocusAdapter {
        @Override
        public void focusGained(FocusEvent e) {
            /*
             * After a formatted text field gains focus, it replaces its text
             * with its current value, formatted appropriately of course.
             * It does this after any focus listeners are notified. We want to
             * make sure that the caret is placed in the correct position rather
             * than the dumb default that is before the 1st character
             */
